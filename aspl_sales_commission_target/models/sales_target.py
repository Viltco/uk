...
#         dstart = datetime.strptime(self.date_start, '%Y-%m-%d')
    ...
    def _check_target_status(self):
        ...
        for each in self:
            ...
            if each.target_lines and all([line.target_state != 'open' for line in each.target_lines]):
    ...
    check_lines_state = fields.Boolean(string="Check Target", compute="_check_target_status", store=True)
    ...
    def create(self, vals):
        ...
        if res:
            ...
            res.message_post(body=_('Hello %s, you have new sales target: %s') % (res.user_id.name, res.name),
