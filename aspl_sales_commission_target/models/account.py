        ...
        for rec in self:
            for invoice in rec.invoice_ids:
                if invoice.type != 'out_refund':
                    ...
                    origin_invoice_id = self.env['account.move'].search([('name', '=', invoice.ref.split()[2].split(',')[0])], limit=1)
                    ...
                    if invoice.type == 'sales_person':
                        ...
                        commission_account_id = IrDefault.get('res.config.settings', "sales_person_commission_account_id", company_id=self.env.user.company_id.id) or False
