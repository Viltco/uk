...
#                                                'company_id': self.env.user.company_id.id})._get_default_journal()
...
class wizard_commission_summary(models.TransientModel):
    ...
    type = fields.Selection([('sales_person', 'Sales Person'), ('consultant', 'Consultant'),
                             ('distributor', 'Distributor'), ('all', 'All')], string="Type", default='sales_person', required=True)
    ...
    def print_commission_report(self):
        ...
        return self.env.ref('aspl_sales_commission_target.report_print_commission_summary').report_action(self, data=datas)
    ...
    def send_commission_report(self):
        ...
        if self.mail_to and self.template_id:
            ...
            pdf_data = self.env.ref('aspl_sales_commission_target.report_print_commission_summary').render_qweb_pdf(self, data=datas)
            if pdf_data:
